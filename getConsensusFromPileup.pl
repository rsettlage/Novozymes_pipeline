...
my ($cmd, $helpFlag, $header, $pileupFn, $samFn, $noGapFlag, $refFn, $conPosFlag, $coverageExp, $outFn);
$cmd = "$0 @ARGV";
...
if($noGapFlag && !defined $refFn){
	print STDERR "\nThe '-nogap' option requires a reference file with the '-ref' option\n"; help(1); 
}
...
sub help{
	...
	print STDERR " Usage: $0 <pileup file>  <sam file> \n"
		...
		."\t[-nogap (a flag to concatenate all blocks on a chromosome)  -ref <reference FASTA file>]\n"
...
while(<$in>){
	...
	if($rPos - 1 > $prePos){	#### if there is a gap...
		if(!$noGapFlag){
			printBlock() if($prePos != 0);
			...
			$conSeq .= substr($refSeq, $prePos, ($rPos-$prePos-1));
			$qPos+= ($rPos-$prePos-1);
		}
	}
	...
	if($prePos != $rPos){ 
		...
		if($mID != -1 && $id2Base[$mID] ne 'N'){
			...
			#print "$_\n$depth >= $minDepth, if($base2ID{$rBase} != $mID  && $max > $readCnt_all* $minRate_varPrint){  \n";
			if($depth >= $minDepth){
		...
		#if(length($conBase) > 2){	 print "$_\n"; exit 1;		}
		if( !defined $conBase ) {print "$_\n"; exit 1;}
		$conBase = $rBase if($conBase eq "N");
		$conSeq .= $conBase;
			...
			if($#indels > -1){
				...
				{				
					...
					if($indelReadCnt >= $readCnt_all * $minRate_indelReplace){ ### if it is a contigs, it takes any gaps supported by at least 20% of reads.
						if($indelType eq "+" && $indelStr !~ /NN/ && $indelStr ne "N"){
							$conSeq .= $indelStr;
		...
		################################## for INDELs		
		#if($conSeq =~ /NN/){ print "$_\nconBase: $conBase\n". substr($conSeq, length($conSeq)-20) . "\n"; exit;}
	}
	$prePos = $rPos;
}
...
sub printTailGap{	
	if(!$noGapFlag && defined $ref && $refLen{$ref} - $prePos > 1){		
...
sub printBlock{
	if(!$noGapFlag && $conSeq ne "" && $isGapPrinted == 1 && $bRightPrinted == 0){ 
		print $outGap "  right : " . substr($conSeq, 0, 100) . "\n\n";
	}
	elsif($noGapFlag){
		my $len = length($refSeq)-$rPos;
		$conSeq .= substr($refSeq, $prePos, $len);
		#print "last : substr(\$refSeq, $prePos, $rPos-".length($refSeq).");\n";
		$qPos += $len;
	}
