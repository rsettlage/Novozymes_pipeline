...
sub help{
	...
	print STDERR "  ex) $0 -ori ori.seq  -g final.genome.sam  -s final.genome.reads.sam   -o out.txt\n\n";
	exit($return);
}
...
while(<$in>){
	...
	if($refName ne $arr[2]){
		#print " +++ $arr[2]\n";
		while($l < $cntL){ 
			print $out "$loci[$l]{tName}\t$loci[$l]{tStart}\t$refName\t$loci[$l]{start}\t$loci[$l]{ori}\t$loci[$l]{new}\t$loci[$l]{sup}/$loci[$l]{tot}\n";
			$l++;
		}
